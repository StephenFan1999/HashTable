...
public class HashTable<K extends Comparable<K>, V> implements HashTableADT<K, V> {
	...
	private DataPair<K,V> hashTable[][]; // a 2D array of DataPair objects with key and value
	...
	public HashTable() {
		...
		this.tableSize = 11;
		this.loadFactorThreshold = 1.50;
		this.numKeys = 0;
		this.hashTable = new DataPair[tableSize][6];
	}
	...
	private void rehash() {
		...
		for (int i = 0; i < hashTable.length; i++) {
			for (int j = 0; j < hashTable[i].length; j++) {
				...
				if (addMeBack != null) {
					int updatedIndex = getIndex(addMeBack.key);
	...
	/**
	 * helper method to get hash index
	 * @param key is the key you want the hash index of
	 * @return the reduced hash index
	 */
	private int getIndex(K key) {
		// gets the original hashcode of the key
		int hashCode = Math.abs(key.hashCode()); 
		
		// use key mod tablesize to get reduced hash index
